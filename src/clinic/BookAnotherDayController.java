...
import tablesClasses.P_check;
import tablesClasses.P_phones;
 ...
 * @author Owner
 */
public class BookAnotherDayController implements Initializable {
    ...
    @FXML
    private DatePicker DayofBooking;
    ...
    private void Done(ActionEvent event) {
        ...
        String date = dtf.format(DayofBooking.getValue()).concat(time);
        int visit_id = 0;
        String visit_type = VType.getValue().toString();
        if (visit_type.equals("كشف")) {
            visit_id = 1;
        } else if (visit_type.equals("استشارة")) {
            visit_id = 2;
        }
